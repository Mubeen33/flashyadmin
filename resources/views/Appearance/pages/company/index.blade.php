...
<div class="content-body">
            ...
            <div class="card form-group">
              <div class="card-body">
                <div class="row mb-2">
                  <div class="col-9">
                    <h4>Custom Company Pages<span class="ml-2 text-light" style="font-size: 10px;">(Drag the rows to change the position.)</span></h4>
                  </div>
                  <div class="col-3">
                    <a class="btn btn-primary float-right" href="{{route('admin.company.create')}}">Add New Page</a>
                  </div>
                </div>
                <table class="table table-bordered demo-dt-basic" cellspacing="0" width="100%" id="myTable">
                <thead>
                    <tr>
                ...
                <tbody>
                    @foreach(\App\Page::where('page_type' , 'C')->orderBy('position')->get() as $key => $page)
                        <tr>
                            <td class="id" style="display: none;">{{$page->id}}</td>
                            <td class="index">{{$key+1}}</td>
                            <td class="text-left">{{$page->title}}</td>
                            <td class="text-left slug">{{ route('custom-pages.show_custom_page', $page->slug) }}</td>
                            <td>
                            ...
                            <td>
                                <div class="btn-group dropdown">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-right">
                                        <li><a class="dropdown-item" href="{{route('admin.company.edit', $page->id)}}">Edit</a></li>
                                        <li>
                                    ...
                                    <form id="destroy" action="{{ route('admin.company.destroy' , $page->id) }}" method="POST" style="display: none;">
                                      @method('DELETE')
                                        @csrf
                                    </form>
                                    </li>
