...
class WarrantyController extends Controller
{
    ...
    public function delete_warranty($id){
        $data = ProductsWarranty::where('id', decrypt($id))->first();
        if (!$data) {
            return redirect()->back()->with('msg','<div class="alert alert-error" id="msg">Warranty Record Not Found!</div>');
        }
        $data->delete();
        return redirect("admin/get-products-warranties")->with('msg','<div class="alert alert-success" id="msg">Warranty Deleted Successfully!</div>');
    }
}
